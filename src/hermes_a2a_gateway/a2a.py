...
class A2AClient:
    def __init__(self, settings: Settings):
        ...
        headers = {"A2A-Version": "1.0", "User-Agent": f"hermes-a2a-gateway/{__version__}"}
        if settings.token:
            headers["Authorization"] = f"Bearer {settings.token}"
        self._client = httpx.AsyncClient(timeout=timeout, headers=headers, follow_redirects=False, trust_env=False)
    ...
    def _http_error(exc: httpx.HTTPStatusError) -> A2AError:
        ...
        if status == 429:
            return A2AError("a2a_rate_limited", "Hermes rate limited the broker", retryable=True)
        if status == 404:
