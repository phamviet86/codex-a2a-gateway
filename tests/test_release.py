...
def publisher() -> ModuleType:
    path = Path(__file__).parents[1] / "scripts/publish_release.py"
...
def valid_run(version: str) -> dict[str, Any]:
    return {
        "id": 123,
        "name": "CI",
        "status": "completed",
...
def test_publication_orders_verification_before_publish(
    ...
    monkeypatch.setattr("hermes_a2a_gateway.__version__", publisher.VERSION)
    dist = tmp_path / "dist"
    dist.mkdir()
...
def test_existing_download_checks_distribution_version(publisher: ModuleType, tmp_path: Path, version: str) -> None:
    ...
    with zipfile.ZipFile(tmp_path / publisher.ASSETS[0], "w") as wheel:
        wheel.writestr("hermes_a2a_gateway-0.5.1.dist-info/METADATA", f"Name: hermes-a2a-gateway\nVersion: {version}\n")
    with tarfile.open(tmp_path / publisher.ASSETS[1], "w:gz") as archive:
        content = f'[project]\nname = "hermes-a2a-gateway"\nversion = "{publisher.VERSION}"\n'.encode()
...
def test_release_workflow_checks_out_successful_main_push_sha(publisher: ModuleType) -> None:
    ...
    for guard in (
        ...
        "contains(github.event.workflow_run.head_commit.message, '[release v')",
