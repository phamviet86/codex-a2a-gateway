...
```text
codex-a2a-gateway 0.5.1
```

The `v0.5.1` wheel includes setup/usage skills and direction-specific readiness as well as the durable Hermes plugin, timeout recovery, `INPUT_REQUIRED` continuation and optional inbound execution preferences.
...
`a2a_call` is Hermes' synchronous convenience tool. The published `v0.5.1` wheel includes this plugin; install it and enable only its separate CLI toolset:
...
## 7. Upgrade from v0.5.0 to v0.5.1

Stop the existing gateway and close clients owning its MCP process before upgrading. Preserve the state files and current launcher/client settings; v0.5.1 adds schema 6 remote-binding lineage without discarding existing data. Back up stopped state according to the ownership rules above. Install the new wheel into the same environment, then install the packaged skills:
...
Stop the v0.5.1 gateway and close MCP writers before rollback. Preserve a consistent backup of the stopped database and its WAL/SHM. Reinstall v0.5.0, then start only that version:
...
|---|---|
| `--version` is not `0.5.1`, or wheel installation fails | Repeat the manifest-verified install above. Do not use a cached or differently named wheel as a substitute. |
