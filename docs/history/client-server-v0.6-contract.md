# Client/server v0.6 implementation contract

Status: implementation approved 2026-09-20. This additive beta preserves all
v0.5 commands, tools and databases. It does not claim exactly-once execution or
...
packaging, shared documentation and installation scripts. Existing legacy modules
remain compatible. New commands are `broker`, `client`, `client-mcp` and
...
- `POST /v1/operations`: `{operation_id, flow_id, prompt, artifact_ids: []}`.
  Client-generated UUIDs identify operation and flow. An accepted response is an
...
environment-provided encryption key and explicit TTL. Legacy prompt retention
does not change. Digests support idempotency, but plaintext must not enter logs,
...
Keep legacy service and state separate; rollback stops the new units and removes
only the new MCP registration. Deployment must not restart unrelated tasks.
