...
Hướng dẫn này dành cho một người dùng cài `codex-a2a-gateway` `v0.5.1` trên **một máy local** rồi kiểm tra cả hai chiều. Release đã gồm plugin durable, recovery timeout, tiếp tục `INPUT_REQUIRED` và extension model/reasoning tùy chọn. Mỗi chiều có một vai trò riêng:
...
`a2a_call` built-in của Hermes là call đồng bộ. Wheel `v0.5.1` đã có plugin self-contained; cài plugin và chỉ bật toolset riêng `codex_a2a` cho CLI:
