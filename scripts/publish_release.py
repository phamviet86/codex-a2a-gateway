...
def verify_download(directory: Path) -> None:
    ...
    with zipfile.ZipFile(directory / ASSETS[0]) as wheel:
        ...
        require(metadata["Name"] == "hermes-a2a-gateway" and metadata["Version"] == VERSION, "wheel version mismatch")
    with tarfile.open(directory / ASSETS[1]) as archive:
        ...
        require(project["name"] == "hermes-a2a-gateway" and project["version"] == VERSION, "sdist version mismatch")
...
def main() -> None:
    ...
    from hermes_a2a_gateway import __version__
    ...
    require(not published["draft"] and published["prerelease"] == PRERELEASE, "release is not published")
