#!/usr/bin/env python3
"""Publish v0.5.1 from exactly the successful main/push CI commit, never a frozen older SHA."""
...
def main() -> None:
    ...
    if api(f"{prefix}/git/ref/tags/{TAG}", optional=True) is None:
        tag = api(
            f"{prefix}/git/tags",
            {"tag": TAG, "message": f"{TAG} Hermes continuation compatibility", "object": sha, "type": "commit"},
        )
